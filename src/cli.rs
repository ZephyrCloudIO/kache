...
pub fn run_gc_local(config: &Config, mode: GcMode) -> Result<()> {
    ...
    // Automatic age-retention pass (kunobi-ninja/kache#4): this is the
    // unattended path (the wrapper's detached background GC, and the CLI's
    // no-daemon fallback), so — like the daemon's periodic sweep — it
    // composes `gc_max_age_hours` on top of size-pressure eviction rather
    // than requiring an operator to type `--max-age`. `0` disables it.
