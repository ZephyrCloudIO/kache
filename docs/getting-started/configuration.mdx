...
| `KACHE_MAX_SIZE` | `cache.local_max_size` | `50GiB` | Maximum local store size, measured as **registered blob content bytes** (each deduplicated blob counted once, not once per entry referencing it; index and metadata overhead sit outside the sum). GC evicts down to 90% of this cap |
