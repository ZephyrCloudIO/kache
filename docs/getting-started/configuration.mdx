...
| `KACHE_MAX_SIZE` | `cache.local_max_size` | `50GiB` | Maximum local store size, measured as **registered blob content bytes** (each deduplicated blob counted once, not once per entry referencing it; index and metadata overhead sit outside the sum). GC has a real 10% hysteresis band: it fires once physical usage exceeds this cap (100%) and evicts down to 90% of it, rather than firing and stopping at the same line |
...
| `KACHE_MIN_STORE_COMPILE_MS` | `cache.min_store_compile_ms` | `0` | Put-side admission control: a compile faster than this many milliseconds is not stored — a sub-second rebuild rarely earns its disk. `0` (default) stores everything, preserving prior behavior. A separate knob from `KACHE_MIN_COMPILE_MS`, which gates remote *prefetch* download decisions, not local storage |
| `KACHE_GC_MAX_AGE_HOURS` | `cache.gc_max_age_hours` | `168` (7 days) | Age-retention hours applied automatically by every unattended GC sweep (the daemon's periodic sweep, the wrapper's background auto-GC, and a bare `kache gc`), composed with size-pressure eviction rather than replacing it. `0` disables automatic age eviction. An explicit `kache gc --max-age <duration>` is unaffected: it always means exactly what was typed |
...
| `KACHE_INCREMENTAL_CRATES` | `cache.incremental_crates` | `[]` | Crate-scoped `preserve_incremental`: the listed crates always pass through with rustc incremental preserved (no cache lookup, no store), regardless of the adaptive heuristic's state. For edit-loop-hot leaf crates whose compile-loop cadence outruns the adaptive policy's learning window. Entries match rustc's `--crate-name` with `-`/`_` treated as equal, so a Cargo package name works. Env value is comma/whitespace-separated and replaces the file list |
...
| `KACHE_REMOTE_RESTORE_TIMEOUT_SECS` | `cache.remote_restore_timeout_secs` | `300` | Total deadline for one on-demand remote restore. On elapse the daemon drops the transfer and reports a miss, so rustc recompiles locally instead of waiting on a stalled remote. The transport's connect and read-inactivity timeouts already bound dead endpoints; this catches slow-drip transfers. `0` = no deadline |
| `KACHE_REMOTE_NEGATIVE_TTL_SECS` | `cache.remote_negative_ttl_secs` | `60` | How long the daemon remembers a definitive remote miss (404 only — never timeouts, 5xx, or credential errors) so parallel wrappers demanding the same absent key don't stampede S3. A successful upload of the key clears its entry immediately. `0` disables the negative cache |
