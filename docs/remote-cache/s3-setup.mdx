...
<Callout type="warn">
This precedence is for the wrapper and one-shot CLI commands (`kache sync`, `kache doctor`, `kache stats` run directly). The **daemon** (`kache daemon run`) resolves *which remote* (bucket/endpoint/region/prefix/profile) from `[cache.remote]` in the config file only — never from environment, regardless of who started it (kunobi-ninja/kache#706). If you want remote caching to work through the daemon (uploads, warm prefetch, daemon-served remote checks), put those fields in `~/.config/kache/config.toml`. `KACHE_S3_ACCESS_KEY`/`KACHE_S3_SECRET_KEY` are unaffected by this fix and are still read from whichever process's ambient environment performs S3 I/O — for the daemon, that means the daemon's own environment, same as before. Prefer a named profile (`profile` / `KACHE_S3_PROFILE`) or the standard AWS credential chain (IAM role, `~/.aws/credentials`) for the daemon so its credentials don't depend on which process happened to start it either.
</Callout>
